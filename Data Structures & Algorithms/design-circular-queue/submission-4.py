...
class MyCircularQueue:
    ...
    def enQueue(self, value: int) -> bool:
        if self.leng < self.cap: 
            ...
            print(f'the value of last node is {last.val}')
    ...
    def deQueue(self) -> bool:
        ...
        if self.leng > 0: 
            print(f'current length {self.leng}')
            root = self.left
            while root.next:
                print(root.next.val)
                root = root.next 
            print(f'the value of self.left.next.next is {self.left.next.next.val}')
            print(f'self.left.next.next is {self.left.next.next}')
